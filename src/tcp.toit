...
class TcpFramer implements Framer:
  ...
  read reader/reader.BufferedReader -> Frame?:
    ...
    transaction_id := binary.BIG_ENDIAN.uint16 header 0
    length := binary.BIG_ENDIAN.uint16 header 4
    unit_id := binary.BIG_ENDIAN.uint8 header 6
    function_code := binary.BIG_ENDIAN.uint8 header 7
    data := reader.read_bytes length - 2
    return Frame --transaction_id=transaction_id --unit_id=unit_id --function_code=function_code --data=data
  ...
  write frame/Frame writer:
    ...
    binary.BIG_ENDIAN.put_uint16 bytes 0 frame.transaction_id
    binary.BIG_ENDIAN.put_uint16 bytes 4 frame.data.size + 2
    binary.BIG_ENDIAN.put_uint8 bytes 6 frame.unit_id
